...
Quoting a recall figure from a handful of rows would be worse than quoting
none. What the harness does guarantee is that the denominator, once labelled,
will have been found by something other than the rules being scored — and that
even then it is recall *relative to the sweep's reach*, never true recall,
since a disclosure phrased in words neither matcher contains is invisible to
both.

## 10. Limitations

1. **Recall is unmeasured.** §9 says why; the harness is built and its pool
   emitted. This is the largest single gap in the phase.
