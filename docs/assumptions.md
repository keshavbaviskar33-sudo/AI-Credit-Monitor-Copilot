...
| A-06 | Core line items (revenue, current assets/liabilities, total assets, debt, equity, operating cash flow, interest expense) can be mapped from US-GAAP XBRL concepts for most in-scope companies | Ratio coverage | **Medium→Low-Medium — measured Phase 4, and it is worse than assumed** | Coverage measured per concept over the 12-filing golden set ([data_dictionary.md §6.1](data_dictionary.md)) | **Partly confirmed:** 5 concepts are 12/12, but `Liabilities` 6/12, `LongTermDebt` 7/12, `Revenues` 9/12 and `SalesRevenueNet` **0/12**. A single-tag lookup is not viable; Phase 5 needs fallback chains and explicit derivation rules |
