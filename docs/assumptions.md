...
| A-16 | A single structured LLM call can produce a grounded synthesis when given evidence with IDs | Keeps the synthesis layer simple | Medium | Grounding validator + evaluation suite, Phase 12 | **Still open after Phase 12, and half of it is now testable.** The validator the assumption's validation plan called for exists and is measured — 0.000 false-positive rate, 100% detection across seven fault classes on 176 assessments ([grounded_synthesis.md §5](grounded_synthesis.md)) — so *whether a draft is grounded* is now a decidable question. What is unmeasured is the assumption itself: no live call has been made (no `anthropic` package, no API key, no CLI credential in this environment), so the real-model acceptance rate is unknown. `scripts/phase12_synthesize.py` produces it from one command and one key ([D-043](decision_log.md)) |
