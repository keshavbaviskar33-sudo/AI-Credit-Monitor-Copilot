...
| A-06 | Core line items (revenue, current assets/liabilities, total assets, debt, equity, operating cash flow, interest expense) can be mapped from US-GAAP XBRL concepts for most in-scope companies | Ratio coverage | Medium — companies use different concepts and custom extensions | Measure mapping coverage per field on a sample in Phase 3/5 | Open |
| A-07 | Text-based PDFs of 10-K financial statements can be obtained or generated for a golden evaluation set | Needed to measure extraction accuracy against XBRL | Medium | Assemble golden set in Phase 4 | Open |
...
| A-10 | MD&A and Risk Factors sections can be located in 10-K HTML reliably enough for NLP | NLP input quality | Medium — item headings vary | Section-detection accuracy on a sample, Phase 10 | Open |
